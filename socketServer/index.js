...
io.on("connection",(socket)=>{
   ...
   socket.on("identity",async (userId)=>{
    try {
      ...
      console.log("identity sync failed", error?.message || error)
    }
   }) 
   ...
   socket.on("update-location",async ({userId,latitude,longitude})=>{
    try {
      ...
      console.log("location update failed", error?.message || error)
    }
   })
  ...
  socket.on("send-message",async (message)=>{
    try {
      ...
      console.log("send message failed", error?.message || error)
    }
  })
