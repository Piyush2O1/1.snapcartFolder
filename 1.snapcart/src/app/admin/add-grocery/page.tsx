...
formData.append("image",backendImage)
   ...
   router.replace("/admin/groceries")
    ...
    if(axios.isAxiosError(error)){
        setErrorMessage(error.response?.data?.message || "Unable to add grocery")
    }else{
        setErrorMessage("Unable to add grocery")
    }
} finally {
    setLoading(false)
}
    ...
    return (
        <div className='min-h-screen flex items-center justify-center bg-linear-to-br from-green-50 to-white py-16 px-4 relative'>
            <Link href={"/admin/dashboard"} className='absolute top-6 left-6 flex items-center gap-2 text-green-700 font-semibold bg-white px-4 py-2 rounded-full shadow-md hover:bg-green-100 hover:shadow-lg transition-all'>
                ...
                <form className='flex flex-col gap-6 w-full ' onSubmit={handleSubmit}>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
                        <div >
                            ...
                            <select name="category" value={category} className='w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-green-400 transition-all bg-white' onChange={(e)=>setCategory(e.target.value)} suppressHydrationWarning>
